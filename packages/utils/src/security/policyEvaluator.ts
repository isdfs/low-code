...
export function policyEvaluator(
   ...
   policies: Record<string, (user: Record<string, any>) => boolean>
) {
   function evaluate(policyName: string): boolean {
       const policyEvaluator = policies[policyName];
       if (policyEvaluator) {
           return policyEvaluator(userAttributes);
       }
       console.warn(`Policy '${policyName}' not found`);
       return false;
   }

   function addPolicy(policyName: string, evaluator: (user: Record<string, any>) => boolean) {
