...
export class I18nManager {
  ...
  translate(key: string, params?: Record<string, string | number>): string {
      const translation = this.localeData[this.currentLocale][key];
      if (!translation) {
          return key;
      }
      return translation.replace(/{(\w+)}/g, (_, paramKey) => {
          return params && params[paramKey] !== undefined ? String(params[paramKey]) : `{${paramKey}}`;
      });
  }
  ...
  formatCurrency(amount: number, currency: string): string {
      return new Intl.NumberFormat(this.currentLocale, { style: 'currency', currency }).format(amount);
  }
}
